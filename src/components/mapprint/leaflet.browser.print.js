...
L.BrowserPrint = L.Class.extend({
  ...
  initialize: function (map, options) {
    ...
    if (
      ...
      !map.getPane(this.options.customPrintStyle.pane)
    ) {
  ...
  _printAuto: function (mode) {
    ...
    var orientation;
    if (
      mode.options.orientation === "Portrait" ||
      mode.options.orientation === "Landscape"
    ) {
  ...
  _removePrintClassFromContainer: function (map, printClassName) {
    ...
    if (
      container.className &&
      container.className.indexOf(printClassName) > -1
    ) {
    ...
    if (this.options.custom) {
      ...
      if (this.options.custom.rectangle) {
        this.options.custom.rectangle.setBounds(
          L.latLngBounds(this.options.custom.start, e.latlng)
        );
      } else {
  ...
  _endAutoPolygon: function (e) {
    ...
    if (this.options.custom && this.options.custom.rectangle) {
      ...
      var orientation;
      if (
        this.options.custom.mode.options.orientation === "Portrait" ||
        this.options.custom.mode.options.orientation === "Landscape"
      ) {
      ...
      this._print(
        this._getMode(orientation, this.options.custom.mode),
        autoBounds
      );
  ...
  _setupPrintPagesWidth: function (pagesContainer, size, pageOrientation) {
    pagesContainer.style.width =
      pageOrientation === "Landscape" ? size.Height : size.Width;
  },

  _setupPrintMapHeight: function (
    mapContainer,
    size,
    pageOrientation,
    options
    ...
    var header =
      ...
      options.header.size &&
      !options.header.overTheMap
        ? options.header.size + " - 1mm"
        : "0mm";
    var footer =
      options.footer &&
      options.footer.enabled &&
      options.footer.size &&
      !options.footer.overTheMap
        ? options.footer.size + " - 1mm"
        : "0mm";
  ...
  _print: function (printMode, autoBounds) {
    ...
    this._map.fire(L.BrowserPrint.Event.PrePrint, {
      printLayer: origins.printLayer,
      printObjects: origins.printObjects,
      pageOrientation,
      printMode: options.mode,
      pageBounds: origins.bounds,
    });
    ...
    this._map.fire(L.BrowserPrint.Event.PrintStart, {
      printLayer: origins.printLayer,
      printMap: overlay.map,
      printObjects: overlay.objects,
    });
  ...
  _completePrinting: function (overlayMap, origins, printObjects) {
    ...
    setTimeout(function () {
      ...
      self._map.fire(L.BrowserPrint.Event.Print, {
        printLayer: origins.printLayer,
        printMap: overlayMap,
      ...
      if (printPromise) {
        Promise.all([printPromise]).then(function () {
          ...
          self._map.fire(L.BrowserPrint.Event.PrintEnd, {
            printLayer: origins.printLayer,
            printMap: overlayMap,
            printObjects: printObjects,
          });
  ...
  _addPrintCss: function (pageSize, pageMargin, pageOrientation) {
    ...
    switch (pageOrientation) {
      case "Landscape":
        printStyleSheet.innerText +=
          " @page { size : " + pageSize + " landscape; }";
        break;
      default:
      case "Portrait":
        printStyleSheet.innerText +=
          " @page { size : " + pageSize + " portrait; }";
        break;
    }
  ...
  _addPrintMapOverlay: function (printMode, pageOrientation, origins) {
    ...
    this.__overlay__.appendChild(
      this._addPrintCss(pageSize, pageMargin, pageOrientation)
    );
    ...
    return this._setupPrintMap(
      overlayMapDom.id,
      this._combineBasicOptions(origins.printLayer),
      origins.printLayer,
      origins.printObjects,
      origins.panes
    );
  },

  _combineBasicOptions: function (printLayer) {
    var options = L.BrowserPrint.Utils.cloneBasicOptionsWithoutLayers(
      this._map.options
    );
  ...
  _setupPrintMap: function (id, options, printLayer, printObjects, panes) {
    ...
    panes.forEach(function (p) {
      overlayMap.createPane(p.name, p.container);
    });
    var clones = {};
    var popupsToOpen = [];
    ...
    for (var p = 0; p < popupsToOpen.length; p++) {
      ...
      if (popupModel.source) {
        ...
        if (element && element.bindPopup && element.openPopup) {
          clones[popupModel.source._leaflet_id]
            .bindPopup(popupModel.popup)
            .openPopup(popupModel.popup.getLatLng());
        }
      }
    }
  ...
  _appendControlStyles: function (container) {
    ...
    printControlStyleSheet.innerHTML +=
